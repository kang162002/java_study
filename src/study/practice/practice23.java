...
public class practice23 {
	...
	public static void prac1() {
		 ...
		 * 
		 * System.out.print("문자 입력 : "); String s1 = scanner.nextLine();
		 * 
		 * System.out.print("숫자 입력 : "); //123 int i1 = scanner.nextInt();
		 * 
		 * scanner.nextLine(); //기존에 남아있는 \n 엔터 청소용 \n
		 * 
		 * System.out.print("문자 입력 : "); String s2 = scanner.nextLine();
		 */
...
//			System.out.println("3. 바닐라라떼  4300원");
			...
			if (addOrder.equals("n")) {
				...
				System.out.println("====================");
				// System.out.printf("총액 : %d원", menu1Count * 3500 + menu2Count * 4100 +
				// menu3Count * 4300);
			...
			/*
			 * //정상 메뉴 선택 여부! 1,2,3 //if(menu ==1 || menu == 2 || menu == 3) if(menu>=1 &&
			 * menu <=3) { //정상범위
			 * 
			 * //수량 입력 System.out.print("수량 선택 : "); int count = scanner.nextInt();
			 * 
			 * scanner.nextLine(); //clear
			 * 
			 * System.out.print("추가주문 하시겠습니까? (y/n) : "); String addOrder =
			 * scanner.nextLine(); if(addOrder.equals("n")) { break; }
			 * 
			 * } else { System.out.println("잘못입력하셨습니다. 다시 입력하세요!!!"); }
			 */
...
//		int menu3Count = 0;
		...
		while (true) { // 참거짓
			...
			/*
			 * if(menu == 1) countArr[menu-1] += count; else if (menu == 2) countArr[menu-1]
			 * += count; else if (menu == 3) countArr[menu-1] += count;
			 */
			...
			if (addOrder.equals("n")) {
				...
				for (int i = 0; i < menuArr.length; i++) {
					if (countArr[i] > 0) { // countArr[i] 보다 크면 > 0 보여주도록 출력한다
